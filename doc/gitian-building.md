...
*Setup instructions for a gitian build of csports using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the csports
...
- [Building csports](#building-csports)
...
Clone the git repositories for csports and gitian and then checkout the csports version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the csports repository
...
To build csports (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the csports repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/csports/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/csports/csports
...
For example:
```bash
URL=https://github.com/konez2k/csports.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit csports=${COMMIT} --url csports=${URL} ../csports/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit csports=${COMMIT} --url csports=${URL} ../csports/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit csports=${COMMIT} --url csports=${URL} ../csports/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/csports-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/csports-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/csports-build.assert
```
...
[csports/gitian.sigs](https://github.com/csports/gitian.sigs/) repository, or if that's not possible to create a pull
request.
