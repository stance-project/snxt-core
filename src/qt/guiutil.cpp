...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CSPN address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no csports: URI
...
{
    // Convert csports:// to csports:
    //
    //    Cannot handle this later, because csports:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open csports.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "csports.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for csports.lnk
...
{
    return GetAutostartDir() / "csports.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a csports.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=csports\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the csports app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add csports app to startup item list
